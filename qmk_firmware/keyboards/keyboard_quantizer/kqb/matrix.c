...
void qt_matrix_init(void) {
    ...
    setPinInputHigh(KQB_PIN_SW0);
    setPinInputHigh(KQB_PIN_SW1);
    setPinInputHigh(KQB_PIN_SW2);
    setPinOutput(KQB_PIN_LED0);
    setPinOutput(KQB_PIN_LED1);
    setPinOutput(KQB_PIN_LED2);
...
uint32_t qt_matrix_scan(matrix_row_t *matrix_raw) {
    ...
    if (led_on_count) {
        ...
        writePinHigh(KQB_PIN_LED2);
    } else {
        writePinLow(KQB_PIN_LED0);
        writePinLow(KQB_PIN_LED2);
    }
    ...
    if (ch559_start) {
        ...
        matrix_row_t row1_1 = (row1 & ~0x000007) | (readPin(KQB_PIN_SW0) ? 0 : (1 << 0)) | (readPin(KQB_PIN_SW1) ? 0 : (1 << 1)) | (readPin(KQB_PIN_SW2) ? 0 : (1 << 2));
